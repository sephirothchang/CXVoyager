...
from cxvoyager.process.prechecks import cloudtower_ip, cluster_vip, mgmt_hosts
from cxvoyager.process.prechecks.runner import run_ip_prechecks
from cxvoyager.process.prechecks.types import PrecheckReport, ProbeRecord
from cxvoyager.process.workflow.runtime_context import RunContext
from cxvoyager.process.handlers.prepare import handle_prepare
...
def test_handle_prepare_raises_on_ip_errors(monkeypatch, tmp_path, stage_logger):
    ...
    monkeypatch.setattr("cxvoyager.process.handlers.prepare.create_stage_progress_logger", lambda *args, **kwargs: stage_logger)
    monkeypatch.setattr("cxvoyager.process.handlers.prepare.load_config", lambda *_: {"validation": {"strict": False}})
    monkeypatch.setattr("cxvoyager.process.handlers.prepare.find_plan_file", lambda *_: Path("plan.xlsx"))
    monkeypatch.setattr("cxvoyager.process.handlers.prepare.parse_plan", lambda *_: {"dummy": True})
    monkeypatch.setattr("cxvoyager.process.handlers.prepare.to_model", lambda *_: PlanModel())
    monkeypatch.setattr("cxvoyager.process.handlers.prepare.validate", lambda *_: {"ok": True, "warnings": []})
    monkeypatch.setattr("cxvoyager.process.handlers.prepare.check_dependencies", lambda **kwargs: {})

    error_report = PrecheckReport(records=[ProbeRecord(category="mgmt_host", target="10.0.0.1", level="error", message="失败", probes={})])
    monkeypatch.setattr("cxvoyager.process.handlers.prepare.run_ip_prechecks", lambda *args, **kwargs: error_report)
...
def test_handle_prepare_success_sets_ctx_extra(monkeypatch, tmp_path, stage_logger):
    ...
    monkeypatch.setattr("cxvoyager.process.handlers.prepare.run_ip_prechecks", lambda *args, **kwargs: report)
...
def test_run_ip_prechecks_propagates_has_error(monkeypatch, records, expected):
    ...
    monkeypatch.setattr("cxvoyager.process.prechecks.mgmt_hosts.inspect", lambda *args, **kwargs: [])
    monkeypatch.setattr("cxvoyager.process.prechecks.cluster_vip.inspect", lambda *args, **kwargs: [])
    monkeypatch.setattr("cxvoyager.process.prechecks.storage_ip.inspect", lambda *args, **kwargs: [])
    monkeypatch.setattr("cxvoyager.process.prechecks.cloudtower_ip.inspect", lambda *args, **kwargs: [])
    monkeypatch.setattr("cxvoyager.process.prechecks.bmc_ip.inspect", lambda *args, **kwargs: records)
